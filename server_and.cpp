...
string calculate_and(string s1, string s2);
...
int main(void)
{
  ...
  // send the calculation result to the edge server;
  as_talker(buffer);
...
// The following code are from Beej's tutorial
	int sockfd;
  	...
  	if ((rv = getaddrinfo(NULL, AND_PORT, &hints, &servinfo)) != 0) {
...
// end of the copied code
    ...
    if(is_first){
      cout << "The Server AND is up and running using UDP on port " << AND_PORT << "." << endl;
    ...
    while(1){
      ...
      // transform the data and calculate the result
      string data(buffer);
      ...
      while(getline(ss, tok, ','))
        v.push_back(tok);
      if(v.size() == 1){
        cout << "The Server AND has successfully received " << linecount << " lines from the edge server and finished all AND computations." << endl;
        break;
      }
      
      string res = calculate_and(v[1], v[2]);
      cout << v[1] << " and " << v[2] << " = " << res << endl;

      // store the result
      string send_back = v[0] + "," + v[1] + ",and," + v[2] + "," + res + "\n";
...
string calculate_and(string s1, string s2){
  ...
  for(unsigned i = 0; i < longer.length(); i++){
    if(longer.at(i) == '1' && shorter.at(i) == '1')
      result += "1";
    else
      result += "0";
  }
